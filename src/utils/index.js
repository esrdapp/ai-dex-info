...
export function getSwapLink (token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://app.hpdex.org/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://app.hpdex.org/#/swap?inputCurrency=${token0Address === '0xbe05ac1fb417c9ea435b37a9cecd39bc70359d31' ? 'HPB' : token0Address
      }&outputCurrency=${token1Address === '0xbe05ac1fb417c9ea435b37a9cecd39bc70359d31' ? 'HPB' : token1Address}`
  }
}

export function getMiningPoolLink (token0Address) {
  return `https://app.hpdex.org/#/uni/HPB/${token0Address}`
}

export function getUniswapAppLink (linkVariable) {
  let baseUniswapUrl = 'https://app.hpdex.org/#/uni'
  if (!linkVariable) {
    return baseUniswapUrl
  }

  return `${baseUniswapUrl}/HPB/${linkVariable}`
}
...
export const urls = {
  showTransaction: (tx) => `https://hpbscan.org/tx/${tx}/`,
  showAddress: (address) => `https://hpbscan.org/address/${address}/`,
  showToken: (address) => `https://hpbscan.org/token/${address}/`,
  showBlock: (block) => `https://hpbscan.org/block/${block}/`,
}
